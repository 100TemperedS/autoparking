...
void autodock_controller::final_approach_state_fun(){
    ...
    if (in_view and pose_set.distance > 0.3){
        neuron_backward(jog_distance);
    }
    else{
        neuron_stop();
...
double calc_distance(const boost::array<double, 3>& point1, const boost::array<double, 3>& point2) {
  std::array<double, 3> std_point1 = {point1[0], point1[1], point1[2]};
  std::array<double, 3> std_point2 = {point2[0], point2[1], point2[2]};
  return sqrt(pow(std_point1[0] - std_point2[0], 2) + pow(std_point1[1] - std_point2[1], 2));
}

void autodock_controller::return_to_initial_position(){
    double initial_x = 0.0;
    double initial_y = 0.0;
    double initial_yaw = 0.0;

    double distance = sqrt(pow(initial_x - odom_x, 2) + pow(initial_y - odom_y, 2));
    double angle = atan2(initial_y - odom_y, initial_x - odom_x) - odom_yaw;

    neuron_turn(angle);
    while (fabs(robot_point_temp[2]-robot_point[2]) < fabs(angle)){
        rclcpp::spin_some(shared_from_this());
    }
    neuron_stop();

    neuron_forward(distance);
    while (calc_distance(robot_point_temp, robot_point) < fabs(distance)){
        rclcpp::spin_some(shared_from_this());
    }
    neuron_stop();

    double final_angle = initial_yaw - robot_point[2];
    neuron_turn(final_angle);
    while (fabs(robot_point_temp[2]-robot_point[2]) < fabs(final_angle)){
        rclcpp::spin_some(shared_from_this());
    }
    neuron_stop();

    RCLCPP_INFO(get_logger(), "Returned to initial position!");
    }

void autodock_controller::neuron_backward(double distance){
